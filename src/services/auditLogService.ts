  ...
  const result = await pool.query<AuditLogRow>(
    `
      SELECT
        ...
        al.action,
        al.actor_user_id,
        CASE WHEN au.deleted_at IS NOT NULL THEN 'Deleted Member' ELSE au.name END AS actor_name,
        al.target_user_id,
        CASE WHEN tu.deleted_at IS NOT NULL THEN 'Deleted Member' ELSE tu.name END AS target_user_name,
        al.entity_type,
        al.entity_id,
        al.session_id,
        al.metadata,
