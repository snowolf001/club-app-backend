...
function makeRecoveryCode(): string {
  const seg = () =>
    randomInt(0, 0xffff).toString(16).toUpperCase().padStart(4, '0');
  return `${seg()}-${seg()}-${seg()}`;
}
...
async function main(): Promise<void> {
  ...
  if (existing.length > 0) {
    ...
    const { rows: memberRows } = await pool.query<{ user_id: string }>(
      'SELECT user_id FROM memberships WHERE club_id = $1',
      [oldId]
    );
  ...
  await pool.query(
    `INSERT INTO clubs
     ...
     VALUES ($1, $2, $3, true, 24, 72)`,
    [clubId, CLUB_NAME, JOIN_CODE]
  );
  console.log(`[1/5] Club created: ${CLUB_NAME} (${clubId})`);

  // ── Step 3: Locations ────────────────────────────────────────────────────

  const locationIds: string[] = LOCATIONS.map(() => randomUUID());

  await batchInsert(
    'club_locations',
    ['id', 'club_id', 'name', 'address'],
    LOCATIONS.map((loc, i) => [locationIds[i], clubId, loc.name, loc.address])
  );
  console.log(`[2/5] ${locationIds.length} locations created`);

  // ── Step 4: Users + Memberships ──────────────────────────────────────────
  ...
  for (let week = 51; week >= 0 && sessions.length < 100; week--) {
    ...
    for (const dayOfWeek of weekDays) {
      ...
      const endsAt = new Date(startsAt.getTime() + randomInt(60, 120) * 60_000);
  ...
  const checkinTxRows: unknown[][] = [];
  const checkinCount = new Map<string, number>(
    members.map((m) => [m.membershipId, 0])
  );
  ...
  for (const session of sessions) {
    ...
    for (const member of attendees) {
      ...
      const checkedInAt = new Date(
        session.startsAt.getTime() + randomInt(0, 30) * 60_000
      );
      ...
      checkinCount.set(
        member.membershipId,
        (checkinCount.get(member.membershipId) ?? 0) + 1
      );
    }
  }
  ...
  for (const member of members) {
    ...
    await pool.query(
      'UPDATE memberships SET credits_remaining = $1 WHERE id = $2',
      [remaining, member.membershipId]
    );
  }
